...
We generate the mathematical Complex Baseband (I/Q) arrays before handing them to the hardware. 
...
To advance beyond analog audio, we constructed a digital modulation pipeline. Binary data is mapped to discrete states of phase and amplitude.
\subsection{Frequency-Shift Keying (FSK)}
Frequency-Shift Keying (FSK) represents binary data through discrete changes in the frequency of the carrier wave. Similar to analog FM, we calculate the continuous phase by integrating the discrete frequency shifts. 

\begin{lstlisting}[language=Python, caption={2-FSK Generation via Frequency Integration}]
import numpy as np

def generate_fsk_iq(num_symbols=1000, out_file="fsk.iq", deviation=5000, fs=48000):
    # Map binary data to frequency shifts (-deviation, +deviation)
    data = np.random.randint(0, 2, num_symbols)
    
    # Upsample to create continuous symbols in time
    sps = 10 
    upsampled_data = np.repeat(data, sps)
    freq_signal = np.where(upsampled_data == 0, -deviation, deviation)
    
    # Integrate frequency to get instantaneous phase
    dt = 1.0 / fs
    phase = 2 * np.pi * np.cumsum(freq_signal) * dt
    
    # Generate complex baseband signal
    iq_fsk = np.exp(1j * phase)
    iq_fsk.astype(np.complex64).tofile(out_file)
\end{lstlisting}
\subsection{Binary Phase-Shift Keying (BPSK)}
Binary Phase-Shift Keying (BPSK) was implemented as the fundamental baseline for digital I/Q constellations. It maps binary zeros and ones directly to $180^\circ$ phase shifts (+1 and -1 on the real axis). To confine the infinite bandwidth caused by instantaneous phase switching, a Root Raised Cosine (RRC) pulse-shaping filter was applied.

\begin{lstlisting}[language=Python, caption={BPSK Mapping and RRC Pulse Shaping}]
import numpy as np

def generate_bpsk_iq(num_symbols=1000, out_file="bpsk.iq"):
    # BPSK Constellation Mapping (0 -> -1+0j, 1 -> +1+0j)
    data = np.random.randint(0, 2, num_symbols)
    symbols = np.where(data == 0, -1+0j, 1+0j)
...
\end{lstlisting}
The generated BPSK signals were successfully demodulated over the air with high integrity, validating both the software filtering and hardware phase control.
...
The mathematical theory and execution of the 16-QAM transmitter were entirely flawless. In a simulated, closed-loop wired environment, a GNU Radio receiver perfectly demodulated the signal, displaying 16 distinct, tightly clustered dots on the constellation sink.
...
This was due to Multipath Fading. As the 16-QAM signal propagated through the room, the radio waves reflected off the surroundings. Reflected waves arriving microseconds later altered the phase of the primary signal, while destructive interference altered the amplitude. Because QAM inherently relies on the strict, unaltered preservation of both phase and amplitude to differentiate tightly packed symbols, these room reflections thoroughly scrambled the transmission. Advanced error correction may be needed to perform OTA reliably.

\section{Conclusion}
This work successfully bridged the gap between purely theoretical digital signal processing and physical hardware deployment. Using the Pi-FM hardware architecture evaluated by Aditya and Suraj, we successfully coded and transmitted complex analog and digital Baseband arrays from scratch. Most notably, the 16-QAM deployment served as a practical demonstration of multipath fading.
